...
% pipeline and the Output component emits a reply -- either retrieved (produce-output)
% or assembled from facts (Phase 8 generation); `persist' saves/loads it all.
%
...
\begin{tikzpicture}[
  ...
  \begin{scope}[on background layer]
    \node[draw, dashed, rounded corners, fill=green!5, inner sep=5mm,
          fit=(innet)(assoc)(concept)(attn)(ops)(vec)(gen)] (mem) {};
  \end{scope}
  \node[font=\sffamily\bfseries\footnotesize, anchor=south]
        at (mem.north) {Memory / knowledge stores};

  % ------------------------------------------------- respond / inference ---
  \node[proc, below=34mm of concept] (respond)
        {\textbf{Respond / Ask} \;(inference)\\[2pt]
        {\scriptsize pipeline: generation (describe / why)}\\
        {\scriptsize $\rightarrow$ operation $\rightarrow$ copy $\rightarrow$ compose}\\
        {\scriptsize $\rightarrow$ associative recall}\\
