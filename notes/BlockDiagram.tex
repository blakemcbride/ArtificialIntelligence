...
% Knowledge sources feed the Input component (large files stream in bounded-memory chunks,
% with model caps to bound growth); `learn' writes Hebbian updates into a set of memory
% stores; `respond'/`ask' read those stores through an inference pipeline and the Output
% component emits a reply -- retrieved (produce-output), assembled from facts (generation),
% or continued in-context (induction); `persist' saves/loads it all.
%
...
\begin{tikzpicture}[
  ...
  \node[store, below=of concept] (rel)
        {Relations \;(\texttt{relations})\\{\scriptsize learned connectors / heads (Phase 9)}};
  ...
  \node[store, below=of vec] (gen)
        {Generation store \;(\texttt{generation})\\{\scriptsize facts + transitions $\rightarrow$ describe / why}};

  \begin{scope}[on background layer]
    \node[draw, dashed, rounded corners, fill=green!5, inner sep=5mm,
          fit=(innet)(assoc)(concept)(rel)(attn)(ops)(vec)(gen)] (mem) {};
  \end{scope}
  \node[font=\sffamily\bfseries\footnotesize, anchor=south]
        at (mem.north) {Memory / knowledge stores};

  % ------------------------------------------------- respond / inference ---
  \node[proc, below=14mm of mem] (respond)
        {\textbf{Respond / Ask} \;(inference)\\[2pt]
        {\scriptsize owns: describe / why (generation),}\\
        {\scriptsize \phantom{owns: }continue (induction)}\\
        {\scriptsize else: operation $\rightarrow$ copy $\rightarrow$ compose $\rightarrow$ recall}\\
        {\scriptsize \texttt{ask}: conversation memory + \texttt{infer-answer}}};

  % -------------------------------------------------------- output / reply ---
  \node[io, right=44mm of respond] (output)
        {\textbf{Output component}\\[2pt]
        {\scriptsize \texttt{produce-output} (retrieval)}\\
        {\scriptsize or assembled sentence (generation)}};
